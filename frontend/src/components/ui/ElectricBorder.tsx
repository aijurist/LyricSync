...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  const updateAnim = () => {
    ...
    if (disp) disp.setAttribute("scale", String(50 * (chaos || 1)));
  ...
  // Reduce brightness and intensity for dark mode
  ...
  const glow1Style: CSSProperties = {
    ...
    borderWidth: thickness,
    borderStyle: "solid",
    borderColor: hexToRgba(color, isDark ? 0.4 : 0.8),
    filter: `blur(${isDark ? 0.5 : 1 + thickness * 0.5}px)`,
  ...
  const glow2Style: CSSProperties = {
    ...
    borderStyle: "solid",
    borderColor: color,
    filter: `blur(${isDark ? 1.5 : 4 + thickness * 1}px)`,
  ...
  const bgGlowStyle: CSSProperties = {
    ...
    filter: isDark ? "blur(16px)" : "blur(48px)",
    opacity: isDark ? 0.12 : 0.6,
    zIndex: -1,
    background: `radial-gradient(circle, ${hexToRgba(color, isDark ? 0.2 : 0.9)}, ${hexToRgba(color, isDark ? 0.08 : 0.4)}, transparent)`,
  };
